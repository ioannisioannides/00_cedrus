"""
Comprehensive tests for identity app: authentication, authorization, profiles, roles.
"""
...
class AuthenticationTest(TestCase):
    ...
    def test_login_missing_fields(self):
        ...
        response = self.client.post(reverse("identity:login"), {"username": "testuser"}, follow=True)
        self.assertFalse(response.context["user"].is_authenticated)
...
class DashboardAccessTest(TestCase):
    ...
    def test_cb_admin_cannot_access_other_dashboards(self):
        ...
        response = self.client.get(reverse("identity:dashboard_client"), follow=True)
        self.assertRedirects(response, reverse("identity:dashboard_cb"))
    ...
    def test_lead_auditor_dashboard_access(self):
        ...
        response = self.client.get(reverse("identity:dashboard_auditor"))
        self.assertEqual(response.status_code, 200)
    ...
    def test_client_admin_dashboard_access(self):
        ...
        response = self.client.get(reverse("identity:dashboard_client"))
        self.assertEqual(response.status_code, 200)
    ...
    def test_dashboard_requires_login(self):
        ...
        response = self.client.get(reverse("identity:dashboard"))
        self.assertRedirects(response, f"{reverse('identity:login')}?next={reverse('identity:dashboard')}")

        response = self.client.get(reverse("identity:dashboard_cb"))
        self.assertRedirects(response, f"{reverse('identity:login')}?next={reverse('identity:dashboard_cb')}")
